...
class RGCR(nn.Module):
    ...
    def forward(self, cur_node_emb, c_B, c_L, c_O, c_TW):
        ...
        C_L = self.proj_L(c_L)
        C_O = self.proj_O(c_O)
        C_TW = self.proj_TW(c_TW)

        # 2. Relevance Measurement (Dot product similarity) (Eq. 6)
        s_B = (cur_node_emb * C_B).sum(-1, keepdim=True)
        s_L = (cur_node_emb * C_L).sum(-1, keepdim=True)
        s_O = (cur_node_emb * C_O).sum(-1, keepdim=True)
        s_TW = (cur_node_emb * C_TW).sum(-1, keepdim=True)

        if self.training:
            s_B += torch.randn_like(s_B) * 0.1
            s_L += torch.randn_like(s_L) * 0.1
            s_O += torch.randn_like(s_O) * 0.1
            s_TW += torch.randn_like(s_TW) * 0.1

        # Bound to [0, 1] via Sigmoid (using fast inverted scalar multiplication)
        s_B = torch.sigmoid(s_B * self.scale_inv)
        s_L = torch.sigmoid(s_L * self.scale_inv)
        s_O = torch.sigmoid(s_O * self.scale_inv)
        s_TW = torch.sigmoid(s_TW * self.scale_inv)

        # 3. Feature Weighting & Aggregation (Eq. 7 & 8)
        S_original = self.proj_original(torch.cat([C_B, C_L, C_O, C_TW], dim=-1))
        S_enhanced = (s_B * C_B) + (s_L * C_L) + (s_O * C_O) + (s_TW * C_TW)
        S_unified = S_original + S_enhanced

        # Final reformulated context (Eq. 9)
        return torch.cat([cur_node_emb, S_unified], dim=-1)
...
class TSNR(nn.Module):
    ...
    def forward(self, H, C, coords, cur_nodes, log_d_nn=None):
        ...
        coords: Spatial coordinates [Batch, N, 2]
        cur_nodes: Indices of current nodes [Batch, Starts]
        log_d_nn: Precomputed node-node distance bias [Batch, N, N]
        """
        B, N, D = H.shape
        _, S, _ = C.shape
